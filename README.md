...
## 5c. Số buổi hiện trên dashboard giờ khớp đúng số buổi/tuần khách chọn

Trước đây `scripts/generate-plan-exercises.mjs` chỉ soạn **tối đa 3 "buổi
mẫu"** rồi mặc định khách tự lặp lại trong tuần (Push/Pull/Legs lặp 2 vòng
= 6 buổi thật nhưng dashboard chỉ hiện 3 thẻ) — khiến khách chọn 3, 5 hay
6 buổi/tuần đều thấy y hệt nhau. Giờ số buổi trả về **luôn đúng bằng** số
buổi khách chọn ở quiz:

| Khách chọn ở quiz | Số buổi thật trong tuần | Cách chia (không phải mục tiêu Đốt mỡ) |
|---|---|---|
| 2 buổi | 2 | Full Body × 2 |
| 3 buổi | 3 | Full Body × 3 |
| 4-5 buổi | 5 | Upper → Lower → Push → Pull → Legs |
| 6+ buổi | 6 | Push → Pull → Legs (lặp 2 vòng) |

Riêng mục tiêu **Đốt mỡ** luôn dùng Full Body cho mọi mức buổi/tuần (lấy
cảm hứng từ Darebee: tần suất cao + cardio cuối buổi hợp đốt mỡ hơn tách
nhóm cơ), chỉ khác số buổi lặp lại theo đúng lựa chọn của khách.

Mỗi buổi giờ có thêm **tên chủ đề riêng** (`day_label`: "Upper", "Push"...
— lấy cảm hứng từ cách trình bày lịch tuần của Nourish Move Love) hiện
ngay trên dashboard dạng "Buổi 1 · Upper" thay vì chỉ "Buổi 1" chung
chung, và mô tả gói (`workout_plans.description`) tự nhắc số ngày nghỉ
còn lại trong tuần.

Cần chạy lại `supabase/expand-library-and-schedule-fee.sql` (đã thêm cột
`plan_exercises.day_label`) rồi chạy lại
`node scripts/generate-plan-exercises.mjs` để áp dụng — xem Bước 2 ở mục
5b bên trên. Không cần chạy lại `seed:exercises`.
