...
## 5b. Kho bài tập lớn (thay vì 48 bài mẫu) + đổi lịch tập trả phí

`lib/planLogic.ts` sinh ra đúng **48 tổ hợp `plan_id`** (4 mục tiêu × 3 dụng
cụ × 4 mức số buổi/tuần). `schema.sql` gốc chỉ seed sẵn 5 bài mẫu cho 1
trong 48 gói đó — 47 gói còn lại sẽ trống nếu không tự thêm dữ liệu.
...
**Bước 2 — nạp kho bài tập & tự soạn lịch cho cả 48 gói:**
```bash
# Thêm vào .env.local (Project Settings → API → service_role — KHÔNG commit key này):
# SUPABASE_SERVICE_ROLE_KEY=...

node scripts/seed-exercise-library.mjs        # nạp ~800 bài (public domain, nguồn free-exercise-db)
node scripts/generate-plan-exercises.mjs      # tự soạn lịch riêng cho cả 48 plan_id
```
`generate-plan-exercises.mjs` chọn bài theo dụng cụ được phép của từng
gói (bodyweight / tạ đơn tại nhà / full gym) và chia buổi theo đúng
`splitLabel` (Full Body / Upper-Lower / Push-Pull-Legs), có thể chạy lại
bất cứ lúc nào để "xáo" lại bộ bài — script sẽ xoá và tạo lại
`plan_exercises` của 48 gói này (không đụng đơn hàng/tiến độ khách).
